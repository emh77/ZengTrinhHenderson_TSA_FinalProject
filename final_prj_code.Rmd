...
##Coal Forecasting 
```{r}
#Model 1: deseasonalized differenced coal ARIMA 
...
#Model 2: differenced coal SARIMA
diff_coal_arima_autofit <- auto.arima(ts_coal_diff)
...
#Model 3: forecast with simple exponential smoothing on differenced coal data
...
#Model 4: SS with StructTS() on original data
coal_ss_model <- StructTS(ts_data[,1], type="BSM",fixed=c(NA,NA,NA,NA)) 
...
coal1_scores <- accuracy(diff_des_coal_arima_forecast, ts_test[,1], test=NULL)
coal1_scores<- coal1_scores[,c("RMSE","MAPE")]
coal2_scores <- accuracy(diff_coal_arima_forecast, ts_test[,1], test=NULL)
coal2_scores<- coal2_scores[,c("RMSE","MAPE")]
coal3_scores <- accuracy(diff_coal_ses_forecast, ts_test[,1], test=NULL)
coal3_scores<- coal3_scores[,c("RMSE","MAPE")]
coal4_scores <- accuracy(coal_ss_forecast, ts_test[,1], test=NULL)
coal4_scores<- coal4_scores[,c("RMSE","MAPE")]

coal_scores <- as.data.frame(rbind(coal1_scores[2,], coal2_scores[2,], coal3_scores[2,], coal4_scores[2,]))

coal_score_names<- c("sarima on differenced deseasonalized","arima on differenced","SES on differenced","state space model on original")

coal_scores<- cbind(coal_score_names, coal_scores)
rm(coal1_scores, coal2_scores, coal3_scores, coal4_scores)
```
...
df_2012_2022<- df_train %>% filter(year(date) > 2011)
...
#Model 2: apply SARIMA on original data 
...
#Model 3: forecast with simple exponential smoothing on original data
...
#Model 4: SS with StructTS()
gas_ss_model <- StructTS(ts_data_2012[,2], type="BSM",fixed=c(NA,NA,NA,NA)) 
...
```{r}
#test the accuracy of the models 
gas1_scores <- accuracy(des_gas_arima_forecast, ts_test[,2], test=NULL)
...
gas4_scores<- accuracy(gas_ss_forecast$mean, ts_test[,2], test=NULL)
gas4_scores<- gas4_scores[,c("RMSE","MAPE")]

gas_scores <- as.data.frame(rbind(gas1_scores[2,], gas2_scores[2,], gas3_scores[2,], gas4_scores))
gas_score_names<- c("arima on deseasonalized","sarima on original","SES on original", "state space on original")
gas_scores<- cbind(gas_score_names, gas_scores)
rm(gas1_scores, gas2_scores, gas3_scores, gas4_scores)
```
...
og_hydro_arima_autofit <- auto.arima(ts_train[,3])
print(og_hydro_arima_autofit)
#forecast with auto arima on original data 
og_hydro_arima_forecast <- forecast(object = og_hydro_arima_autofit, h = 12)
plot(og_hydro_arima_forecast)

#Model 3: forecast with simple exponential smoothing on original data
og_hydro_ses_forecast=ses(y = ts_train[,3], h = 12, holdout = FALSE, silent = FALSE)  
checkresiduals(og_hydro_ses_forecast)
plot(og_hydro_ses_forecast, type="l")
...
```{r}
#test the accuracy of the models 
hydro1_scores <- accuracy(des_hydro_arima_forecast, ts_test[,3], test=NULL)
hydro1_scores<- hydro1_scores[,c("RMSE","MAPE")]
hydro2_scores <- accuracy(og_hydro_arima_forecast, ts_test[,3], test=NULL)
hydro2_scores<- hydro2_scores[,c("RMSE","MAPE")]
hydro3_scores <- accuracy(og_hydro_ses_forecast, ts_test[,3], test=NULL)
hydro3_scores<- hydro3_scores[,c("RMSE","MAPE")]
hydro4_scores<- accuracy(hydro_ss_forecast$mean, ts_test[,3])
hydro4_scores<- hydro4_scores[,c("RMSE","MAPE")]

hydro_scores <- as.data.frame(rbind(hydro1_scores[2,], hydro2_scores[2,], hydro3_scores[2,], hydro4_scores))
hydro_score_names<- c("arima on deseasonalized","sarima on original","SES on original", "state space on original")
hydro_scores<- cbind(hydro_score_names, hydro_scores)
rm(hydro1_scores, hydro2_scores, hydro3_scores, hydro4_scores)
```
...
#Model 2: apply SARIMA on original data 
og_nuclear_arima_autofit <- auto.arima(ts_train[,4])
print(og_nuclear_arima_autofit)
#forecast with auto arima on original data 
og_nuclear_arima_forecast <- forecast(object = og_nuclear_arima_autofit, h = 12)
plot(og_nuclear_arima_forecast)

#Model 3: forecast with simple exponential smoothing on original data
og_nuclear_ses_forecast=ses(y = ts_train[,4], h = 12, holdout = FALSE, silent = FALSE)  
checkresiduals(og_nuclear_ses_forecast)
plot(og_nuclear_ses_forecast, type="l")
...
```{r}
#test the accuracy of the models 
nuclear1_scores <- accuracy(des_nuclear_arima_forecast, ts_test[,4], test=NULL)
nuclear1_scores<- nuclear1_scores[,c("RMSE","MAPE")]
nuclear2_scores <- accuracy(og_nuclear_arima_forecast, ts_test[,4], test=NULL)
nuclear2_scores<- nuclear2_scores[,c("RMSE","MAPE")]
nuclear3_scores <- accuracy(og_nuclear_ses_forecast, ts_test[,4], test=NULL)
nuclear3_scores<- nuclear3_scores[,c("RMSE","MAPE")]
nuclear4_scores<- accuracy(nuclear_ss_forecast$mean, ts_test[,4])
nuclear4_scores<- nuclear4_scores[,c("RMSE","MAPE")]

nuclear_scores <- as.data.frame(rbind(nuclear1_scores[2,], nuclear2_scores[2,], nuclear3_scores[2,], nuclear4_scores))
nuclear_score_names<- c("arima on deseasonalized","sarima on original","SES on original", "state space model")
nuclear_scores<- cbind(nuclear_score_names, nuclear_scores)
rm(nuclear1_scores, nuclear2_scores, nuclear3_scores, nuclear4_scores)
```
...
#Model 4: SS with StructTS() on original 
...
#Model 5: SS with StructTS() on deseasonalized 
wind_ss_model <- StructTS(ts_train[,5], type="BSM",fixed=c(NA,NA,NA,NA)) 
checkresiduals(wind_ss_model)
wind_ss_forecast<- forecast(wind_ss_model, h=12)
plot(wind_ss_forecast)

#Model 6: SS with StructTS() on deseas differenced 
wind_ss_model <- StructTS(ts_train[,5], type="BSM",fixed=c(NA,NA,NA,NA)) 
checkresiduals(wind_ss_model)
wind_ss_forecast<- forecast(wind_ss_model, h=12)
plot(wind_ss_forecast)
...
```{r}
#test the accuracy of the models 
wind1_scores <- accuracy(diff_deseas_wind_arima_forecast, ts_test[,5], test=NULL)
wind1_scores<- wind1_scores[,c("RMSE","MAPE")]
wind2_scores <- accuracy(diff_wind_sarima_forecast, ts_test[,5], test=NULL)
wind2_scores<- wind2_scores[,c("RMSE","MAPE")]
wind3_scores <- accuracy(diff_coal_ses_forecast, ts_test[,5], test=NULL)
wind3_scores<- wind3_scores[,c("RMSE","MAPE")]
wind4_scores<- accuracy(wind_ss_forecast$mean, ts_test[,5])
wind4_scores<- wind4_scores[,c("RMSE","MAPE")]

wind_scores <- as.data.frame(rbind(wind1_scores[2,], wind2_scores[2,], wind3_scores[2,], wind4_scores))
wind_score_names<- c("arima on diff deseas","sarima on differenced","SES on differenced", "state space on original")
wind_scores<- cbind(wind_score_names, wind_scores)
rm(wind1_scores, wind2_scores, wind3_scores, wind4_scores)
```