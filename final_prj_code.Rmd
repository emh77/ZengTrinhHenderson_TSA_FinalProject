...
A summary of the final data is broken down below. This table details the minimum, maximum, and average generation by source as well as the first 10 months of data.
...
The training data df was then converted to a time series for initial ACF and PACF analysis. The descriptions below summarize results captured in the plots for each source's time series, ACF, and PACF that follow.
...
This report captures results for Seasonal ARIMA, Seasonal Exponential Smoothing, and Seasonal State Space models. Analysis of model performance is broken down by generation source below.
...
The State Space (StructTS) model performs best for coal as shown in the table of accuracy scores. Although these values are lowest for StructTS, the ACF of residuals suggests the Exponential Smoothing model may better capture the trends as it has fewer significant lags and thus is truly random. 
...
#forecast
coal_sses_forecast<-forecast(coal_sses, h=12, start=c(2008,1))
plot(coal_sses_forecast)
...
The State Space (StructTS) model performs best for gas as shown in the table of accuracy scores. However, the ACF of residuals of the Exponential Smoothing model once again suggests it better captures trends. It shows no significant lags whereas the StructTS model shows many, thus capturing more than a random component. The SARIMA model outperforms the SES model on some accuracy parameters. 
...
The SES model in general performs best for hydro as shown in the table of accuracy scores. Although these values are lowest for SES, the histogram of residuals suggests the StructTS model may better capture the trends as it has a more normal distribution instead of a right tail. 
...
The SARIMA model performs best for nuclear as shown in the table of accuracy scores. The residuals for SES and StructTS show a similar number of significant ACF lags and distorted (ie not distinctly normal) histograms suggesting the forecasted models do not capture all of the trends.  

```{r echo=FALSE, message=FALSE, warning=FALSE}
#Model 1: SARIMA
nuclear_sarima_autofit <- auto.arima(ts_train[,4])
print(nuclear_sarima_autofit)
checkresiduals(nuclear_sarima_autofit, plot=TRUE)
#forecast
